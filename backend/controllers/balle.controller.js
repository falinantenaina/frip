import Balle from "../models/balle.model.js";
import Depense from "../models/depense.model.js";
import Produit from "../models/produit.model.js";
import Vente from "../models/vente.model.js";

export const getBalles = async (req, res, next) => {
  try {
    const { statut, search } = req.query;
    let query = {};
    if (statut) query.statut = statut;
    if (search)
      query.$or = [
        { nom: { $regex: search, $options: "i" } },
        { numero: { $regex: search, $options: "i" } },
      ];
    const balles = await Balle.find(query).sort({ createdAt: -1 });
    res.status(200).json({ success: true, count: balles.length, data: balles });
  } catch (error) {
    next(error);
  }
};

export const getBalle = async (req, res, next) => {
  try {
    const balle = await Balle.findById(req.params.id);
    if (!balle)
      return res
        .status(404)
        .json({ success: false, message: "Balle non trouvée" });
    const produits = await Produit.find({ balle: balle._id });
    const ventes = await Vente.find({ balle: balle._id }).populate("livreur");
    const depenses = await Depense.find({ balle: balle._id });
    res
      .status(200)
      .json({ success: true, data: { balle, produits, ventes, depenses } });
  } catch (error) {
    next(error);
  }
};

export const createBalle = async (req, res, next) => {
  try {
    const balle = await Balle.create(req.body);
    res.status(201).json({ success: true, data: balle });
  } catch (error) {
    next(error);
  }
};

// FIX : utiliser .save() pour que calculerBenefice soit appelé si prixAchat change
export const updateBalle = async (req, res, next) => {
  try {
    const balle = await Balle.findById(req.params.id);
    if (!balle)
      return res
        .status(404)
        .json({ success: false, message: "Balle non trouvée" });

    // Appliquer les champs modifiables
    const fields = [
      "nom",
      "numero",
      "prixAchat",
      "dateAchat",
      "description",
      "statut",
    ];
    fields.forEach((f) => {
      if (req.body[f] !== undefined) balle[f] = req.body[f];
    });

    // Recalculer le bénéfice (important si prixAchat a changé)
    balle.calculerBenefice();
    await balle.save();

    res.status(200).json({ success: true, data: balle });
  } catch (error) {
    next(error);
  }
};

export const deleteBalle = async (req, res, next) => {
  try {
    const balle = await Balle.findById(req.params.id);
    if (!balle)
      return res
        .status(404)
        .json({ success: false, message: "Balle non trouvée" });
    const ventesCount = await Vente.countDocuments({ balle: balle._id });
    if (ventesCount > 0)
      return res
        .status(400)
        .json({
          success: false,
          message:
            "Impossible de supprimer une balle avec des ventes associées",
        });
    await Produit.deleteMany({ balle: balle._id });
    await Depense.deleteMany({ balle: balle._id });
    await balle.deleteOne();
    res.status(200).json({ success: true, data: {} });
  } catch (error) {
    next(error);
  }
};

// FIX : recalcul complet depuis les ventes et dépenses réelles
export const recalculateStats = async (req, res, next) => {
  try {
    const balle = await Balle.findById(req.params.id);
    if (!balle)
      return res
        .status(404)
        .json({ success: false, message: "Balle non trouvée" });

    const ventes = await Vente.find({
      balle: balle._id,
      statutLivraison: { $ne: "annulé" },
    });
    balle.totalVentes = ventes.reduce((sum, v) => sum + v.prixVente, 0);
    balle.nombreVentes = ventes.length;

    const depenses = await Depense.find({ balle: balle._id });
    balle.depensesLiees = depenses.reduce((sum, d) => sum + d.montant, 0);

    balle.calculerBenefice();
    await balle.save();

    res.status(200).json({ success: true, data: balle });
  } catch (error) {
    next(error);
  }
};

export const getBallesStats = async (req, res, next) => {
  try {
    const stats = await Balle.aggregate([
      {
        $group: {
          _id: null,
          totalBalles: { $sum: 1 },
          totalInvesti: { $sum: "$prixAchat" },
          totalVentes: { $sum: "$totalVentes" },
          totalDepenses: { $sum: "$depensesLiees" },
          totalBenefice: { $sum: "$benefice" },
        },
      },
    ]);
    const statsByStatus = await Balle.aggregate([
      { $group: { _id: "$statut", count: { $sum: 1 } } },
    ]);
    res.status(200).json({
      success: true,
      data: {
        global: stats[0] || {
          totalBalles: 0,
          totalInvesti: 0,
          totalVentes: 0,
          totalDepenses: 0,
          totalBenefice: 0,
        },
        byStatus: statsByStatus,
      },
    });
  } catch (error) {
    next(error);
  }
};
